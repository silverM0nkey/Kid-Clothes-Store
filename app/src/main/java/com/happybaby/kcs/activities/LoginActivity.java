                    ...
                    callGetCustomer.enqueue(new CallbackWithRetry<ResponseCustomer>(activity) {
                        ...
                        public void onResponse(Call<ResponseCustomer> call, Response<ResponseCustomer> response) {
                            if (response.isSuccessful()) {
                                ...
                                List<ShoppingCartProduct> productList = AppDatabase.getInstance(activity).shoppingCartDao().findProductsByCustomer(CustomerProfile.CUSTOMER_ANONYMOUS);

                                for (ShoppingCartProduct product: productList){
                                    product.setCustomer(responseCustomer.getEmail());
                                }
                                AppDatabase.getInstance(activity).shoppingCartDao().insertAll(productList);
                                AppDatabase.getInstance(activity).shoppingCartDao().deleteProductsByCustomer(CustomerProfile.CUSTOMER_ANONYMOUS);
                                finish();
                            } else {
